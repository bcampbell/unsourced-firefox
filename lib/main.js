...
UnsourcedState.prototype.lookupFinished = function(lookupResults) {
  ...
  if(this.lookupState=="none" || this.lookupState=="pending") {
    ...
    if(this.lookupResults.labels) {
      this.labels = this.lookupResults.labels;
    }
    if(this.lookupResults.sources) {
      this.sources = this.lookupResults.sources;
    }
      ...
      if( response.status==200) {
        ...
        if(out.status=='success') {
          ...
          if(state.lookupState=='ready') {
            ...
            state.labels = state.lookupResults.labels;
...
UnsourcedState.prototype.badSmellsFound = function() {
  if(this.pageDetails) {
...
UnsourcedState.prototype.calcWidgetTooltip = function() {
  ...
  switch( this.lookupState ) {
    case "none":
      if(this.badSmellsFound()) {
        tooltip_txt = "Article might have problems";
      }
      break;
    case "pending":
      tooltip_txt = "checking - please wait";
      break;
    case "ready":
      {
        var ad = this.lookupResults;
      ...
      break;
    case "error":
      tooltip_txt = "error while checking";
      break;
  }
  ...
  if(state.lookupState=="ready" && state.contentReady==true && state.labelDisplayLatch!==true) {
    if( state.labels ) {
      if(options.show_overlays) {
        worker.port.emit('showWarningLabels', state.labels);
    ...
    onAttach: function(worker) {
      ...
      worker.port.on('pageExamined', function (pageDetails) {
        ...
        if(state.lookupState=='none') {
          ...
          if( state.pageDetails.isDefinitelyArticle === true || state.badSmellsFound() ) {
