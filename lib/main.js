...
{
  console.log("update_gui: ", state.getDebugTxt());
  if( state===undefined ) {
    console.log("Background: TRYING TO UPDATE ON NON-TRACKED PAGE");
  }

  unsourcedWidget.port.emit('reconfig', {'icon': state.calcWidgetIconState()});

  // if the tab is active, update the popup window
  if(tabs.activeTab===worker.tab) {
    unsourcedPopup.port.emit('bind', state);
  }
}
...
unsourcedPopup.on('show', function() {
  ...
  if(state === undefined) {
    unsourcedPopup.port.emit('bind', null);
  } else {
    unsourcedPopup.port.emit('bind', state);
  }
});
